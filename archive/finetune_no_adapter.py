...
# CUDA_VISIBLE_DEVICES=0 python -m mrl_eval.hf.finetune_no_adapter --dataset hebnli 2>&1 | tee mrl_log_mt5_xl_no_adapter.txt
# CUDA_VISIBLE_DEVICES=1 python -m mrl_eval.hf.finetune_no_adapter --dataset hebnli --model "google/gemma-2-2b" 2>&1 | tee mrl_log_gemma2_2b_no_adapter.txt
# CUDA_VISIBLE_DEVICES=2 python -m mrl_eval.hf.finetune_no_adapter --dataset hebnli --model "google/gemma-3-4b-it" 2>&1 | tee mrl_log_gemma3_4b_no_adapter.txt
# CUDA_VISIBLE_DEVICES=3 python -m mrl_eval.hf.finetune_no_adapter --dataset hebnli --model "dicta-il/dictalm2.0" 2>&1 | tee mrl_log_dictalm2_no_adapter.txt

# CUDA_VISIBLE_DEVICES=0 python -m mrl_eval.hf.finetune --dataset hebnli --model "dicta-il/dictalm2.0" 2>&1 | tee mrl_log_dictalm2.txt
...
# CUDA_VISIBLE_DEVICES=2 python -m mrl_eval.hf.finetune --dataset hebnli --model "google/gemma-3-12b-it" 2>&1 | tee mrl_log_gemma3_12b.txt
  ...
  def on_evaluate(
    ...
    if self.metric_for_best_model:
      ...
      if is_first_checkpoint or self._is_better_metric(decision_metric):
        ...
        save_path = pathlib.Path(self.output_dir) / f"{self.best_checkpoint}"
...
def finetune_decoder_only(
  ...
  eval_callback = DecoderEvalAndSaveCallback(
      ...
      save_full_model=True,  # NEW PARAMETER - Set to True to save full merged model
  )
